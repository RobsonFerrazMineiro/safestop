-- ============================================================================
-- SafeStop — Sprint 2.4: Ver e Agir (start evaluation + record decision + timeline)
-- ============================================================================
-- Referências: docs/decisions/VER-E-AGIR-DECISIONS.md (PO-1…PO-20)
-- SEM tabela occurrence_evaluations; SEM notification_events; SEM IO.
-- ============================================================================
...
create function public.start_occurrence_evaluation(p_occurrence_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
comment on function public.start_occurrence_evaluation(uuid) is
  'PP → EM_AVALIACAO; idempotente para mesmo evaluator (VER-E-AGIR PO-17).';
...
create function public.record_occurrence_decision(p_payload jsonb)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if not found then
    raise exception 'Conflito ao atualizar status da ocorrência %', v_occurrence_id;
  end if;
...
comment on function public.record_occurrence_decision(jsonb) is
  'EM_AVALIACAO → VER_E_AGIR; whitelist decision_type=VER_E_AGIR (Sprint 2.4).';
...
revoke insert, update, delete on public.occurrence_decisions from authenticated;
grant execute on function public.start_occurrence_evaluation(uuid) to authenticated;
grant execute on function public.record_occurrence_decision(jsonb) to authenticated;


-- ============================================================================
-- 4. Patch get_occurrence_timeline — títulos PO-20 + metadata decisão
-- ============================================================================
...
comment on function public.get_occurrence_timeline(uuid, jsonb, integer) is
  'Feed timeline unificado; títulos Ver e Agir PO-20 (Sprint 2.4).';
