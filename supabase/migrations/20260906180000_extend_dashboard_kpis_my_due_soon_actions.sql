-- ============================================================================
-- SafeStop — Extensão controlada: personal.myDueSoonActions em get_dashboard_kpis
-- ============================================================================
-- Mesma fórmula temporal de managerial.dueSoonActionItems (due_at >= now() e
-- due_at <= now() + p_due_soon_days), restrita a responsible_member_id = v_member_id.
-- Não altera assinatura de parâmetros nem demais KPIs.
-- ============================================================================
...
declare
  ...
  v_my_due_soon_actions integer := 0;
...
begin
  ...
  if v_member_id is not null then
    ...
    into v_my_pending_actions, v_my_overdue_actions, v_my_due_soon_actions
  ...
  return jsonb_build_object(
    'personal', jsonb_build_object(
      ...
      'myDueSoonActions', coalesce(v_my_due_soon_actions, 0),
...
comment on function public.get_dashboard_kpis(uuid, integer, timestamptz, timestamptz) is
  'KPIs de estoque/fluxo do Dashboard (Sprint 3.2 + personal.myDueSoonActions). SECURITY DEFINER com gates has_permission() explícitos.';
